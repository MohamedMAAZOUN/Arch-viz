...
describe("resolveSyncAccess", () => {
  it("grants the owner edit access", async () => {
    ...
    expect(await resolveSyncAccess(repo, owner, project.id)).toEqual({
      role: "owner",
      canEdit: true,
    });
  });
  ...
  it("grants an editor member edit access", async () => {
    ...
    expect(await resolveSyncAccess(repo, editor, project.id)).toEqual({
      role: "editor",
      canEdit: true,
    });
  });
  ...
  it("makes a viewer member read-only", async () => {
    ...
    expect(await resolveSyncAccess(repo, viewer, project.id)).toEqual({
      role: "viewer",
      canEdit: false,
    });
  });
  ...
  it("makes a public project read-only for a non-member", async () => {
    ...
    expect(await resolveSyncAccess(repo, stranger, project.id)).toEqual({
      role: "viewer",
      canEdit: false,
    });
  });
  ...
  it("denies access to a missing project", async () => {
    ...
    expect(
      await resolveSyncAccess(repo, someone, "00000000-0000-0000-0000-000000000000"),
    ).toBeNull();
  });
});
